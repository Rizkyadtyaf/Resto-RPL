...
class RouteCollection implements RouteCollectionInterface
{
	...
	{
		$this->defaultNamespace = filter_var($value, FILTER_UNSAFE_RAW
	);
	...
	{
		$this->defaultController = filter_var($value, FILTER_UNSAFE_RAW
	);
	...
	{
		$this->defaultMethod = filter_var($value, FILTER_UNSAFE_RAW
	);
	...
	public function resource(string $name, array $options = null): RouteCollectionInterface
	{
		...
		{
			$newName = ucfirst(filter_var($options['controller'], FILTER_UNSAFE_RAW
		));
		}
	...
	protected function create(string $verb, string $from, $to, array $options = null)
	{
		...
		$from = filter_var($prefix . $from, FILTER_UNSAFE_RAW
	);
